...
get_next_hop(uint8_t dest_id, uip_ipaddr_t *next_hop)
{
  switch(dest_id) {
    ...
    case 2: set_ipaddr(next_hop, 3); break; // via 7 (Link-local)
    case 3: set_ipaddr(next_hop, 3); break; // direct (Link-local)
    case 4: set_ipaddr(next_hop, 4); break; // via 8 (Link-local)
    case 5: set_ipaddr(next_hop, 3); break; // via 2 (Link-local)
    case 6: set_ipaddr(next_hop, 3); break; // direct (Link-local)
    case 7: set_ipaddr(next_hop, 3); break; // via 3 (Link-local)
